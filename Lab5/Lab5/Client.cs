...
namespace Client
{
    ...
    {// The port number for the remote device.  
        ...
        public static String StartClient(State stateObj)
        {
        ...
        private static void ConnectCallback(IAsyncResult ar)
        {
            try
            {
                ...
                State stateObj = (State)ar.AsyncState;
        ...
        private static void Receive(State stateObj)
        {
            try
            {
                // Begin receiving the data from the remote device.  
                stateObj.WorkSocket.BeginReceive(stateObj.Buffer, 0, State.BUFFER_SIZE, 0,
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    client.BeginReceive(state.Buffer, 0, State.BUFFER_SIZE, 0,
                        new AsyncCallback(ReceiveCallback), state);
                }
                else
                {
                    // All the data has arrived; put it in response.  
                    if (state.sBuilder.Length > 1)
                    {
                        responses[state.Index] = state.sBuilder.ToString();
                    }
        ...
        private static void Send(State stateObj, String data)
        {
        ...
        private static String FormatResult(State stateObj)
        {
